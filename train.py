...
def train(model, dataloader, criterions, epoch, recorder, args, optimizer, scheduler, warm_up=1, refine=False):
...
# loss function for embedding
criterions = [eval(l)(args, recorder, device) for l in args.embed_loss]
...
# train graph refine
args.emb_feat_path = checkpoint_path
exp_lr_scheduler = Scheduler(optimizers, step_size=20, gamma=0.1)
trainset = eval(args.refine_dataset)(args)

print('==> Start Training Refinement')
model.train_refine = True
criterions = [eval(l)(args, recorder, device) for l in args.refine_loss]
print("criterion:", ", ".join([type(cri).__name__ for cri in criterions]))
...
for epoch in range(0, args.epochs):
    ...
    # training in 1 epoch
    train(model, trainloader, criterions, epoch, recorder, args, optimizer, exp_lr_scheduler, warm_up=warm_up, refine=True)
    exp_lr_scheduler.step() #adjust the optimizers learning rate
