...
from models.model import FeatureBlock
...
class rk_two_view_net(nn.Module):
    def __init__(self, args):
        super(rk_two_view_net, self).__init__()

        class_num = args.nclasses
        droprate = args.drop
        stride = args.stride
        pool = args.pool
        share_weight = args.share
        VGG16 = args.VGG16
        ...
        self.class_head = None #self.classifier.classifier #save head to resume training
        self.classifier = FeatureBlock(pool_dim, low_dim, dropout=drop)
    # def eval(self):
    #     if self.class_head is None:
    #         self.class_head = self.classifier.classifier #save head to resume training
    #     self.classifier.classifier = nn.Sequential() #remove clasifier head in evaluatin
    
    #     return self.train(False)

    # def train(self, mode=True):

    #     if mode and self.class_head is not None: #train
    #         self.class_head, self.classifier.classifier = None, self.class_head
    #     elif not mode and self.class_head is None:
    #         self.class_head, self.classifier.classifier = self.classifier.classifier, nn.Sequential()
    ...
    def build_opt(self, args):
        ...
        optimizer = optim.SGD([
                    ...
                    {'params': self.classifier.parameters(), 'lr': args.lr_mlp}
    ...
    def forward(self, data):
        ...
        if x1 is None:
            ...
            data['opt']['out_vec'] = self.l2norm(y1)  
        ...
        if x2 is None:
            ...
            data['sar']['out_vec'] = self.l2norm(y2)
...
class three_view_net(nn.Module):
    ...
    def train(self, mode=True):
        self.classifier.classifier = self.class_head 
        self.training = mode
        for module in self.children():
            module.train(mode)
        return self
...
# Test the model, before you train it. 
    net = rk_two_view_net(751, droprate=0.5, VGG16=True)
